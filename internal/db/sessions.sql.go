...
SELECT id, parent_session_id, title, message_count, prompt_tokens, completion_tokens, cost, updated_at, created_at, summary_message_id, todos, tags, goal
FROM sessions
...
const listSessionsByParent = `-- name: ListSessionsByParent :many
SELECT id, parent_session_id, title, message_count, prompt_tokens, completion_tokens, cost, updated_at, created_at, summary_message_id, todos, tags, goal
FROM sessions
...
func (q *Queries) ListSessionsByParent(ctx context.Context, parentSessionID sql.NullString) ([]Session, error) {
	rows, err := q.query(ctx, q.listSessionsByParentStmt, listSessionsByParent, parentSessionID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
const setSessionGoal = `-- name: SetSessionGoal :exec
UPDATE sessions
SET
    goal = ?
WHERE id = ?
...
type SetSessionGoalParams struct {
	Goal sql.NullString `json:"goal"`
	ID   string         `json:"id"`
}

func (q *Queries) SetSessionGoal(ctx context.Context, arg SetSessionGoalParams) error {
	_, err := q.exec(ctx, q.setSessionGoalStmt, setSessionGoal, arg.Goal, arg.ID)
	return err
}

const setSessionTags = `-- name: SetSessionTags :exec
UPDATE sessions
SET
    tags = ?
WHERE id = ?
...
type SetSessionTagsParams struct {
	Tags sql.NullString `json:"tags"`
	ID   string         `json:"id"`
}

func (q *Queries) SetSessionTags(ctx context.Context, arg SetSessionTagsParams) error {
	_, err := q.exec(ctx, q.setSessionTagsStmt, setSessionTags, arg.Tags, arg.ID)
	return err
}
