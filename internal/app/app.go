...
import (
	...
	"github.com/Omerfaruk-aydn/Atlas-Agent/internal/herdr"
...
type App struct {
	...
	// subscribers (notably `Atlas-Agent run` in client/server mode) can
...
func (app *App) setupEvents() {
	...
	setupSubscriber(ctx, app.serviceEventsWG, "background-jobs", shell.GetBackgroundShellManager().Subscribe, app.events)
	if app.Skills != nil {
...
// for `Atlas-Agent run`. A lossy fan-in here can drop the only terminal
