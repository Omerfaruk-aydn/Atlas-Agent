...
func TestLookupConfigs_BoundedByProject(t *testing.T) {
	...
	t.Run("does not pick up atlas.json above non-git project", func(t *testing.T) {
		...
		// atlas.json above the project must not be adopted.
		require.NoError(t, os.WriteFile(
			filepath.Join(parent, "atlas.json"),
			[]byte(`{}`),
			0o644,
		))
		...
		for _, p := range got {
			require.NotEqual(t, filepath.Join(parent, "atlas.json"), p)
		}
	})

	t.Run("does not climb out of git worktree to find atlas.json", func(t *testing.T) {
		...
		strayEval, err := filepath.EvalSymlinks(filepath.Join(parent, "atlas.json"))
		require.NoError(t, err)
		for _, p := range got {
			...
			require.NotEqual(t, strayEval, pEval, "must not adopt parent atlas.json")
		}
	})

	t.Run("picks up atlas.json inside the project", func(t *testing.T) {
		project := t.TempDir()
		local := filepath.Join(project, "atlas.json")
		...
		require.True(t, foundLocal, "expected project atlas.json to be in lookup result: %v", got)
	})
	...
	t.Run("global shell config (atlasrc) is included", func(t *testing.T) {
		project := t.TempDir()

		got := lookupConfigs(project)
		// A global atlasrc is discovered only beside the user config. The data
		// directory is machine-owned state and must never execute a atlasrc.
		require.Contains(t, got, shellConfigSibling(GlobalConfig()))
		require.NotContains(t, got, shellConfigSibling(GlobalConfigData()))
	})

	t.Run("project atlasrc and .atlasrc are discovered", func(t *testing.T) {
		project := t.TempDir()
		require.NoError(t, os.WriteFile(filepath.Join(project, "atlasrc"), []byte(""), 0o644))
		require.NoError(t, os.WriteFile(filepath.Join(project, ".atlasrc"), []byte(""), 0o644))

		got := lookupConfigs(project)
		require.Contains(t, got, filepath.Join(project, "atlasrc"))
		require.Contains(t, got, filepath.Join(project, ".atlasrc"))
	})
	...
	t.Run("system config is loaded first", func(t *testing.T) {
		...
		require.Equal(t, "/etc/Atlas-Agent/atlas.json", got[0])
	})
}
...
// config and a atlasrc coexist in the same directory, the merge warning names
...
func TestLoadFromConfigPaths_ConflictWarningNamesKeys(t *testing.T) {
	...
	t.Run("names overlapping keys", func(t *testing.T) {
		buf := capture(t)
		tmpDir := t.TempDir()
		jsonPath := filepath.Join(tmpDir, "atlas.json")
		rcPath := filepath.Join(tmpDir, "atlasrc")
		...
		require.NoError(t, err)
		require.Contains(t, buf.String(), "atlasrc taking precedence")
	...
	t.Run("no warning when nothing overlaps", func(t *testing.T) {
		...
		_, _, err := loadFromConfigPaths(context.Background(), []string{jsonPath, rcPath})
		require.NoError(t, err)
		require.NotContains(t, buf.String(), "atlasrc taking precedence",
...
func TestConfig_setDefaults(t *testing.T) {
	...
	t.Run("does not adopt .atlas from a parent project", func(t *testing.T) {
		parent := t.TempDir()

		// .atlas in the parent: it should not be reused by the child
	...
	t.Run("does not climb out of git worktree to find .atlas", func(t *testing.T) {
		if _, err := exec.LookPath("git"); err != nil {
			t.Skip("git not available")
		}
		...
		// Stray .atlas above the worktree root.
		require.NoError(t, os.Mkdir(filepath.Join(parent, defaultDataDirectory), 0o755))
		...
		require.NoError(t, err)
		require.NotEqual(t, strayEval, gotEval, "must not adopt parent .atlas")
...
func TestConfig_setupAgentsWithNoDisabledTools(t *testing.T) {
	...
	require.True(t, ok)
	assert.Equal(t, []string{"lsp_symbols", "lsp_definition", "lsp_call_hierarchy", "glob", "grep", "ls", "session_search", "sourcegraph", "usage", "view"}, taskAgent.AllowedTools)
}
...
func TestConfig_setupAgentsWithEveryReadOnlyToolDisabled(t *testing.T) {
	...
	coderAgent, ok := cfg.Agents[AgentCoder]
	require.True(t, ok)
	assert.Equal(t, []string{"agent", "bash", "goal", "atlas_info", "atlas_config", "atlas_logs", "job_output", "job_kill", "download", "edit", "exit_plan_mode", "multiedit", "lsp_diagnostics", "lsp_references", "lsp_restart", "lsp_rename", "lsp_rename_file", "lsp_replace_symbol", "fetch", "agentic_fetch", "orchestrate", "debate", "delegate", "vibe", "facts", "inspect_file", "memory", "question", "skill_manage", "todos", "write", "list_mcp_resources", "read_mcp_resource", "browser", "debugger", "computer", "team_send", "team_read", "dead_code", "type_hierarchy", "import_graph", "impact_analysis", "code_metrics", "todo_scan", "api_surface", "anti_pattern_scan", "security_scan", "generate_docstring", "generate_tests", "semantic_code_search", "metric_export", "env_var_audit", "scan_secrets", "test_run", "coverage_report", "lint_run", "dep_audit", "docker_build_explain", "k8s_manifest_lint", "terraform_lint", "ci_cd_pipeline_debugger", "cloud_resource_costs", "log_tail", "git_status", "git_log", "git_blame", "git_diff", "git_branches", "changelog_gen", "pr_describe", "pre_commit_guard", "git_conventional_commit", "git_conflict_resolver", "audit_trail", "git_commit_split", "github_pr_view", "doc_index"}, coderAgent.AllowedTools)
...
func TestConfig_configureSelectedModels(t *testing.T) {
	t.Run("reload mode should not persist fallback defaults", func(t *testing.T) {
		dir := t.TempDir()
		globalPath := filepath.Join(dir, "atlas.json")
